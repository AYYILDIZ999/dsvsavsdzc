...
client.on("ready", async () => {client.user.setPresence({ activity: { name: config.customstatus }, status: 'idle' }).then(console.log(`Custom status ayarlandı`))});
...
client.on('message', async message => {
  ...
  client.on("messageUpdate", async(oldMessage, newMessage) => { 
      ...
      client.on("ready", async function() {
        const voiceChannel = "809020255976423468"
